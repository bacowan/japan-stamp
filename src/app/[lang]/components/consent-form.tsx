...
export default function ConsentForm({ locale }: ConsentFormParams) {
    ...
    useEffect(() => {
        if (localStorage.getItem("privacy-preferences") === null) {
    ...
    return <>
        {
            ...
            <Modal closeModal={() => setShouldModalShow(false)} className="max-w-[50%] max-h-[75%] p-2 overflow-y-auto">
                ...
                <label><input type="checkbox"/> Opt In</label>
                ...
                <div className="border-t mt-4 w-full flex flex-row justify-around">
                    <button className="m-2 p-2 border rounded-sm">Allow All</button>
                    <button className="m-2 p-2 border rounded-sm">Allow Nessisary Only</button>
                    <button className="m-2 p-2 border rounded-sm">Confirm</button>
                </div>
            </Modal>
        }
    </>
